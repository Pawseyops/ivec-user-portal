...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # these columns in application need to be mapped to new rows in priority_area
        colmap = {
            'priority_area_radio_astronomy': ('Astronomy', 'astronomy', 'Radio Astronomy Priority Area'),
            'priority_area_geosciences': ('Geosciences', 'geosciences', 'Geosciences Priority Area'),
            'priority_area_directors': ('Directors', 'director', 'Directors Priority Area'),
            'priority_area_partner': ('iVEC Partners', 'partner', 'iVEC Partners Priority Area'),
            'priority_area_national': ('National Merit', 'national', 'National Merit Priority Area')
        }
        
        # populate the new priority_area table
        priority_areas = []
        for p in colmap.values():
            pa = orm.PriorityArea(name=p[0], code=p[1], description=p[2])
            pa.save()
            priority_areas.append(pa)
            
        # default System - based on the real one in production
        default_system = orm.System(name='Epic', description='iVEC Supercomputer located at Murdoch University.')
        default_system.save()
        ...
        # set the FK values on the applications
        for a in orm.Application.objects.all():
            a.allocation_round = default_allocation_round
            for old, new in colmap.items():
                if getattr(a, old):
                    print "Setting application %d to code=%s"%(a.id, new[1])
                    a.priority_area = orm.PriorityArea.objects.get(code=new[1])
                    break
            # handle old applications without a priority area at all
            if not a.priority_area_id:
                a.priority_area = orm.PriorityArea.objects.get(code='national')
            a.save()
